...
@ ====================================================================
@ Compute(R0:pointer-to-inputs, R1:end of input value))
@	R0:inputs - expect a sequence of values terminated by -1
@ ====================================================================
Compute:
	stmfd	sp!, {r0-r3,lr} @ YES, we do need this!
	mov	r3, r0       	@ R3:inputs = R0:arg1
NextInput:
	ldr	r0, [r3],#4 	@ R0:input = *(R3:inputs++)
	cmp	r0, r1
	beq	EndCompute   	@ R0:input == end of input value ?

	stmfd	sp!, {r1-r3} @ PUSH {r1,r3} (caller-saves)

	bl	Mystery      @ R0:tmp = Mystery(R0:input)
...
EndCompute:
	ldmfd	sp!, {r0-r3,pc} @ YES, we do need this!
