...
public class HeapPriorityQueue implements PriorityQueue
{
	...
	private void bubbleUp ( ) 
	{
		int s = size();
		while(s>=2){
			if (parent(s) == -1){  //is return from parent
				break;
			}else if (storage[parent(s)].compareTo(storage[s]) > 0){   // >
				swapElement(s,parent(s));
				s=parent(s);
			}else{
				break;
			}
	...
	{	
		for(int i=1;;){
			if(hasLeft(i)){
				if(hasRight(i)){
					if(storage[leftChild(i)].compareTo(storage[i]) <= 0 || storage[rightChild(i)].compareTo(storage[i]) <= 0){
	...
	private int leftChild ( int pos )
	{
		if(isEmpty())
			return -1;
		else if(pos*2 > storage.length)
			return -1;
		else if(storage[pos*2] != null)
			return pos*2;
		else
			return -1;
	}
	...
	{	
		if(isEmpty())
			return -1;
		else if(pos*2 +1 > storage.length)
			return -1;
		else if(storage[pos*2 +1] != null)
			return pos*2 +1;
		else
			return -1;
	}
