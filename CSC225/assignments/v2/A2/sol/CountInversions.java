...
public class CountInversions{
	...
	*/
	public static int CountInversions(int[] A){
		int swaps = 0;                      // assigns swap counter
		int p     = 0;     			        // assigns swap placeholder
		for(int i=(A.length-1);i>0;i--){    // go through array from end (once!)
			p=i;                            // reassigns counter for swapping
			while(A[p-1] > A[p]){			// if needs swap do swaps until good
			    int temp = A[p];			// SWAP:set temp for swap
				A[p] = A[p-1];				// SWAP:make swap
				A[p-1] = temp;				// SWAP:reassign temp
				if(p < (A.length-1)){		// SWAP:incrementer for the swapping
					p++;					// SWAP:increments swap placeholder
				}							// 
				swaps ++;					// increments swap counter
			}								// 
		}									// 
		return swaps;						// returns # of swaps!
	}
