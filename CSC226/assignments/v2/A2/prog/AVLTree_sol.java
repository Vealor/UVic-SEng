...
public class AVLTree{
	...
	*/
	...
	public void insert(TreeNode node,TreeNode n){
		if(root == null){
			...
			if(n.nodeValue.compareTo(node.nodeValue)<=0){
				if(node.leftChild == null){
					node.leftChild = n;
					n.parent = node;
					insertBalance(n);
				}else{
					insert(node.leftChild,n);
				}
			}else{
				if(node.rightChild == null){
					...
					insert(node.rightChild,n);
				}
			}		
		}
	}
	
	private void insertBalance(TreeNode node){
		int balanceNum = getBalance(node);
		if(balanceNum < -1){
			balanceNum = getBalance(node.leftChild);
			if(balanceNum <= 0){
			...
			balanceNum = getBalance(node.rightChild);
			if(balanceNum >= 0){
	...
	*/
	public void remove(TreeNode node){
		TreeNode n = find(node.nodeValue);
		TreeNode r;
		...
		if(n.leftChild==null || n.rightChild == null){
			...
			if(node.rightChild!=null) {
				TreeNode k = node.rightChild;
				while(k.leftChild!=null) {
					k = k.leftChild;
				}
				r=k;
			} else {
				TreeNode m = node.parent;
				while(m!=null && node==m.rightChild) {
					node = m;
					m = node.parent;
				}
				r=m;
	...
	public int height(TreeNode cur){
		...
		if(cur.leftChild==null && cur.rightChild==null) {
			...
			h=height(cur.leftChild);
			hr=height(cur.rightChild);
			if(hr>h)
				h=hr;
			return 1+h;
		}
	...
	private TreeNode rotateLeft(TreeNode node){
		...
		node.parent = r;
		if(r.parent != null){
			if(r.parent.rightChild == node){
				r.parent.rightChild = r;
			}else if(r.parent.leftChild == node){
	...
	private TreeNode rotateRight(TreeNode node){
		...
		TreeNode r = node.leftChild;
		r.parent = node.parent;
		node.leftChild = r.rightChild;
		if(node.leftChild != null){
